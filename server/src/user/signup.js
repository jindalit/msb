...
module.exports.signup = function (req, res) {
    ...
    Role.find({ name: { $in: ['Admin', 'SuperAdmin'] } }).select("_id").exec((err, data) => {
        if (err) {
            ...
            if (data && data.length > 0) {
                ...
                const user = new User({ firstname, lastname, email, password, phone, role, username, isProjectContact: req.isProjectContact, isTechContact: req.isTechContact });
