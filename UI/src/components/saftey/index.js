...
export default class Saftey extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(services.baseUrl + services.getTraining + '?authToken=' + sessionStorage.getItem('authToken') + '&userId=' + JSON.parse(sessionStorage.getItem('user'))._id).then(response => {
            this.setState({
    ...
    handleTraining = (id, status, endDate) => {
        ...
        today = mm + '/' + dd + '/' + yyyy;
        axios.post(services.baseUrl + services.changeTrainingStatus + '?authToken=' + sessionStorage.getItem('authToken'), { 'id': id, statusId: status, userId: JSON.parse(sessionStorage.getItem('user'))._id, startDate: today, endDate: endDate ? today : '' } ).then(response => {
            this.getUserTraining()
        })
    }
    handleLaunch = () => {
        ...
        setTimeout(() => {
            ...
            vid.onended = () => {
                this.handleTraining(this.state.trainingid, '5eaa4eb7a111ad0cfce91ef2',  'endDate')
        ...
        if (e.link) {
            htmlDesc.push(
                <div>
                    ...
                    <p><b>Course Duration:</b> {e.duration}</p>
                    <a onClick={this.handleLaunch} class="request-button align-left">Launch Course</a>
                    ...
                    <div class="jobs-table safety">
                        <div class="container">
                            <div class="jobs-table-inner">
                                <form class="example">
                                    <div class="form-group">
                                        <Autocomplete class="form-control"
                                            ...
                                            onChange={(event, value) => this.setState({ training: value })}
                                ...
                                {this.state.descHtml}
                                {this.state.playVideo ? <div class='backdrop'><div class='videoPlay'>
                                    <video width="940" height="520" id='myvideo' autoPlay controls>
                                        <source src={this.state.link} type="video/mp4" />
                                        <source src={this.state.link} type="video/ogg" />
                                    </video>
                                    <button onClick={e => { this.setState({ playVideo: false }) }}>Close Video</button></div></div> : ''}
