...
export default class MyJobs extends React.Component {
    ...
    componentDidMount() {
        let userId = sessionStorage.getItem('user') ? JSON.parse(sessionStorage.getItem('user'))._id : ''
        axios.get(services.baseUrl + services.historyList + '?authToken=' + sessionStorage.getItem('authToken') + '&userId=' + userId).then(response => {
            this.setState({
    ...
    render() {
        ...
        for (let i = 0; i < this.state.jobsData.length; i++) {
            items.push(
                <tr>
                    <td><Link to={"/jobDetail/" + this.state.jobsData[i]._id}>{this.state.jobsData[i].jobName}</Link></td>
                    <td>{this.state.jobsData[i].createdAt}</td>
                    <td>{this.state.jobsData[i].description}</td>
                    <td>{this.state.jobsData[i].modifiedAt}</td>
